...
class Magik_Civicsettings_Adminhtml_RestoreController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->loadLayout()
            ->_setActiveMenu('magik/civic/restore')
            ->_addBreadcrumb(Mage::helper('civicsettings')->__('Restore Defaults'), Mage::helper('civicsettings')->__('Restore Defaults'));
    ...
    public function indexAction()
    {
        ...
        $this->_title($this->__('Magik'))
            ->_title($this->__('Civic'))
            ->_title($this->__('Restore Defaults'));

        $this->_addContent($this->getLayout()->createBlock('civicsettings/adminhtml_restore_edit'));
    ...
    public function restoreAction()
    {
	    ...
	    try { 
            ...
            $defaults->loadFile(Mage::getBaseDir().'/app/code/local/Magik/Civicsettings/etc/config.xml');
            $this->_restoreSettings($defaults->getNode('default/civicsettings')->children(), 'civicsettings');

		    if ($setup_cms) {
                Mage::getModel('civicsettings/settings')->setupCms();
            }

            Mage::getSingleton('adminhtml/session')->addSuccess(
                Mage::helper('civicsettings')->__('Civic Theme Settings has been restored. Please clear cache (System > Cache management) if you do not see changes in storefront'));
        }
        catch (Exception $e) {
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('civicsettings')->__('An error occurred while restoring theme settings.'));
        }
